...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        Map<Integer,Set<String>> userAlbum = new HashMap<>();




/* 14 */ // Map<Integer, Set<String>> usersAlbum = new HashMap<>(); getAllUser().stream().forEach(user -> { Set<String> albums14 = new HashSet<>(); getAllAlbums().stream().filter(alb ->Album.getUserId() < 4).filter(album -> Album.getUserId() .equals(User.getId())).forEach(album -> {albums14.add(Album.getTitle()); usersAlbum.put(User.getId(), albums14);}); }); usersAlbum.forEach((k, v) -> System.out.println("album: "+k+" - titulos "+v));
